...
| `paper_loop_cn.py` / `paper_status.py` | A股本地虚拟盘常驻调仓 / 本地虚拟盘账户速览 |
| `account_dashboard.py` | 本地账户 HTML 看板：A股虚拟盘 + 美股富途模拟盘/实盘 |
| `scripts/` | Windows 本机静默启动器（pythonw/vbs，避免弹控制台窗口） |
...
3. **A股本地虚拟盘**：`state/paper_account_cn.json`，由 `paper_loop_cn.py`/`paper.py` 驱动，
   只做本地虚拟成交；A股不走富途下单。
