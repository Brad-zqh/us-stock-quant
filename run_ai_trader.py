...
# ---------------------------------------------------------------- 微信报告
def build_wechat_md(res: dict, acc: dict, summ: dict,
                    trades: list[dict], journal: str, regime: str) -> str:
    lines = []
    lines.append(f"更新时间: {res['asof']}")
    if regime:
    ...
    if trades:
        ...
        for t in trades:
            ...
            pnl = f"，盈亏${t['pnl']:,.0f}" if t.get("pnl") else ""
            lines.append(
                f"| {side} | **{t['code']}** | {t.get('name','')} | "
                f"${t['amount']:,.0f} | {t['reason']}{pnl} |")
    else:
    ...
    lines.append("> ⚠️ 全部为模拟演示，仅供研究，非投资建议。")
    return "\n".join(lines)
...
def main() -> int:
    ...
    print(f"自选股: {', '.join(watchlist)}")
    res = engine.analyze(watchlist, period="2y", use_earnings=False)
    ...
    acc = paper.load_account(STATE_PATH)
    trades = paper.rebalance(acc, detail, names=watchlist, reason_regime=regime)
    summ = paper.summary(acc, detail)
    print(f"今日成交 {len(trades)} 笔; 总资产 ${summ['total']:,.0f} ({summ['ret_pct']:+.1f}%)")

    if not dry:
        paper.save_account(acc, STATE_PATH)
        print(f"账户已保存: {STATE_PATH}")

    # 操盘日志: 有大模型 key 用大模型版, 否则免费规则版
    fb = "、".join(f"{r['代码']}{r['盈亏%']:+.0f}%" for r in summ["rows"][:5])
    creds = llm.get_credentials()
    if creds.get("api_key"):
        print(f"使用大模型日志: {creds.get('model')}")
        journal = llm.llm_journal(trades, summ, regime=regime, factors_brief=fb, creds=creds)
    else:
        print("未配置大模型 key, 使用免费规则版日志。")
        journal = llm.rule_based_journal(trades, summ, regime=regime)

    md = build_wechat_md(res, acc, summ, trades, journal, regime)
    title = f"🤖 AI交易员 {res['asof'][:10]}  ${summ['total']:,.0f}({summ['ret_pct']:+.1f}%)"
